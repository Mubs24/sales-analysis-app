...
def process_data(file, filename):
    try:
        ...
        if df.empty:
            return None, "File contains no valid matching rows for 'Date' and 'Sales' after cleaning."
...
def calculate_kpis(df):
    ...
    best_month = monthly_sales.idxmax().strftime('%B %Y') if not monthly_sales.empty else "N/A"
    
    # Growth (comparing last month to previous month if available, else simple metric)
